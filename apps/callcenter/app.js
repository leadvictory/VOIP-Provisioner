...
var app = {
	...
	requests: {
		...
		'callcenter.agent.cdr': {
			'verb': 'GET',
			'url': 'accounts/{accountId}/users/{userId}/cdrs?{filters}'
		},
		'callcenter.agent.cdrId': {
			'verb': 'GET',
...
/*	listDevices: function (callback) {
	...
	format_data: function(data) {
		var self = this,
			formatted_data = {},
			agent_user_id = monster.apps.auth.userId,
			account_priviledge_level = monster.apps.auth.currentUser.priv_level,
			user_agent_session = false;
		/* Formatting Queues */
		...
		$.each(data.queues, function(k, v) {
			formatted_data.queues[v.id] = $.extend(true, {
				...
				abandoned_calls: 0,
				user_queue: false,
				priv_level: account_priviledge_level
			}, v);
		});
		...
		$.each(data.agents, function(k, v) {
			if(agent_user_id == v.id || account_priviledge_level == "admin" ){
				user_agent_session = true;
			}
			if(v.queues && v.queues.length > 0) {
				formatted_data.agents[v.id] = $.extend(true, {
					...
					total_calls: 0,
					queues_list: {},
					agent_session: user_agent_session
				}, v);
			}
			...
			$.each(v.queues, function(k, queue_id) {
				...
				if(agent_user_id == v.id || account_priviledge_level == "admin" ){
					formatted_data.queues[queue_id].user_queue = true;
				}
			});
			user_agent_session = false;
		});
	...
	dashboardUpdateAllData: function($container, data, queueId, callback) {
		...
		data.priv_level = monster.apps.auth.currentUser.priv_level;
	...
	dashboardBindEvents: function($container) {
		...
		$container.find('.js-open-cc-performance').on('click', function(e) {
			e.preventDefault();
			var addRow = false,
				addRowCount = 0;
                        
			self.renderPerformanceIndex($container, addRow, addRowCount);
		});
	},

	//Comment this code properly else you migth go crazy understanding it next time
	renderPerformanceIndex: function(pArgs, addRow, addRowCount) {
		var self = this,
			args = pArgs || {},
			parent = args;

		self.fetchPerformanceData(function(data) {
			var dataTemplate = {
					agents: data,
					add_row_count: addRowCount
				},
			...
			self.performanceInitDatePicker(parent, template, template, addRowCount);

			self.bindReceivedVMs(template, addRowCount);
			
			//For addition of new rows for severeal agents. Needs debugging
			if (addRow){
				parent
					.append(template);
			}else{
	...
	displayQueuesList: function(template, agentId, queues, addRowCount) {
		var self = this;
		container = template;
		
		self.agentQueuesCall(template, queues, function(agentsQueuesList) {
			container.find('.loading-state')
						.hide();
			
			self.fetchPerformanceData(function(data) {
				var dataTemplate = $(monster.template(self, 'performance', {
					agents: data,
					queuesList: agentsQueuesList
				}));

				template.empty().append(dataTemplate);
				template.find('.select-agentbox').val(agentId).trigger('chosen:updated');
	...
	agentQueuesCall: async function(container, queues, callback) {
		var self = this,
			queuesList = [];
			container.find('.loading-state')
			.show();
	...
	fetchPerformanceData: function (callback) {
		var self = this;

		self.get_agents(function(_data_agents) {
			callback(_data_agents);
		});
	},

	//Not current used but might be useful
	format_performance_data: function(data) {
		$.each(data.queues, function(k, v) {
			$.each(data.agents, function(k, a) {
				if(a.queues.includes(v.id)) {
					if (a.queues_list === undefined){
						data.agents[k] = $.extend(true, {
							queues_list: {}
						}, a);
					}
					data.agents[k].queues_list[v.id] = {
						name: v.name
					};
				}
			});
	...
	performanceInitDatePicker: function(template, agentId, queueId, addRowCount) {
		var self = this,
			dates = monster.util.getDefaultRangeDates(self.appFlags.performance.defaultRange),
			fromDate = dates.from,
			toDate = dates.to;

		var optionsDatePicker = {
			container: template,
			range: self.appFlags.performance.maxRange
		};
		...
		template.find('.apply-filter').on('click', function(e) {
			template.find('#cc_chart_container').remove();
			self.displayChart(template, agentId, queueId, addRowCount);
		});
	...
	bindReceivedVMs: function(template, addRowCount) {
		var self = this,
			$selectAgentBox = template.find('.select-agentbox');
			$selectQueueBox = template.find('.select-queuebox');
			$selectRow = template.find('#add-row');
		
		//self.performanceInitDatePicker(template, template);
		...
		monster.ui.chosen($selectAgentBox, {
			placeholder_text_single: self.i18n.active().callcenter.performanceChart.placeholderTextAgent
		});

		monster.ui.chosen($selectQueueBox, {
			placeholder_text_single: self.i18n.active().callcenter.performanceChart.placeholderTextQueue
		});

		$selectAgentBox.on('change', function() {
			var agentId = $(this).val(),
				queues = $(this).find('option:selected').data('foo');
				queues = queues.split(",");
			// We update the select-agentbox from the listing vm messages when we click on a vmbox in the welcome page
			template.find('.select-agentbox').val(agentId).trigger('chosen:updated');

			self.displayQueuesList(template, agentId, queues, addRowCount);
		});

		$selectQueueBox.on('change', function() {
			var queueId = $(this).val(),
				agentId = $selectAgentBox.val();
			// We update the select-agentbox from the listing vm messages when we click on a vmbox in the welcome page
			template.find('.select-agentbox').val(agentId).trigger('chosen:updated');
			template.find('.select-queuebox').val(queueId).trigger('chosen:updated');
			template.find('#cc_chart_container').remove();
			self.performanceInitDatePicker(container, agentId, queueId, addRowCount);
			self.displayChart(template, agentId, queueId, addRowCount);
		});
		...
		$selectRow.on('click', function() {
			...
			self.renderPerformanceIndex(template, addRow, addRowCount);
		});
	},

	displayChart: function(container, agentId, queueId, addRowCount) {
		var self = this,
		...
		container.find('.filter-by-date').toggleClass('active');
		...
		monster.ui.footable(container, {
			getData: function(filters, callback) {
				if (filterByDate) {
					...
					var from = new Date(),
						to = new Date();
						filters = $.extend(true, filters, {
							created_from: monster.util.dateToBeginningOfGregorianDay(from),
							created_to: monster.util.dateToEndOfGregorianDay(to)
						});
				}
				// we do this to keep context
				self.newGetAgentDetails(filters, agentId, queueId, function(data) {
					var i18n = self.i18n.active();
						callsCountData = [
							{
								label: i18n.callcenter.performanceChart.answeredCalls,
								count: data.agents[agentId][queueId].answered_calls,
								color: '#6CC5E9'
							},
							{
								label: i18n.callcenter.performanceChart.missedCalls,
								count: data.agents[agentId][queueId].missed_calls,
								color: '#EF8F25'
							},
						],
					callsTimeData = [
						{
							label: i18n.callcenter.performanceChart.talkTime,
							count: data.agents[agentId][queueId].talk_time,
							color: '#BDE55F'
						},
						{
							label: i18n.callcenter.performanceChart.waitTime,
							count: data.agents[agentId][queueId].wait_time,
							color: '#B588B9'
						},
					],
					dataTemplate = {
						data: data.agents[agentId][queueId],
						callsCountList: _.orderBy(callsCountData, 'count', 'desc'),
						callsTimeList: _.orderBy(callsTimeData, 'count', 'desc'),
						add_row_count: addRowCount
					},
					template = $(self.getTemplate({
					...
					//console.log(container.find('#dashboard_assigned_numbers_chart'));
					template.find('.count-wrapper[data-type="new"] .count-text').html(data.length),
					template.find('.count-wrapper[data-type="total"] .count-text').html(data.length),

					//Chart creation
					//container.find('#dashboard_assigned_numbers_chart').empty()
					$callsCountCanvas = template.find('#dashboard_calls_count_chart'),
					$callsTimeCanvas = template.find('#dashboard_calls_time_chart'),
					emptyDataSet = [
						{
							count: 1,
							color: '#ddd'
					...
					callsCountDataSet = _.sortBy(callsCountData, 'count'),
					callsTimeDataSet = _.sortBy(callsTimeData, 'count'),
					...
					createPieCanvas = function createPieCanvas($target) {
						var args = Array.prototype.slice.call(arguments),
							datasets;
					...
					callsCountDataSet = _.isEmpty(callsCountDataSet) ? emptyDataSet : callsCountDataSet;
					callsTimeDataSet = _.isEmpty(callsTimeDataSet) ? emptyDataSet : callsTimeDataSet;
					...
					createDoughnutCanvas($callsCountCanvas, {
						data: _.map(callsCountDataSet, 'count'),
						backgroundColor: _.map(callsCountDataSet, 'color'),
						borderWidth: 0
					});

					createPieCanvas($callsTimeCanvas, {
						data: _.map(callsTimeDataSet, 'count'),
						backgroundColor: _.map(callsTimeDataSet, 'color'),
						borderWidth: 0
					});
					
					callback && callback(data.agents);
				});
			},
			afterInitialized: function() {
				...
				container.find('.loading-state')
							.hide();
				//var addRowTemplate = container.find('.custom-data-container').show();
				//container.append(addRowTemplate);
			},
			backendPagination: {
				enabled: false
			}
	...
	newGetAgentDetails: function(filters, agentId, queueId, callback) {
		var self = this,
		formatted_data = {};
		formatted_data.agents = {};
		formatted_data.agents[agentId] = {};
		formatted_data.agents[agentId][queueId] = {
			missed_calls: 0,
			answered_calls: 0,
			total_calls: 0,
			wait_time: 0,
		...
		self.getAll({
			...
			success: async function (agent) {
				var agentData = agent.data;
				
				for (i = 0; i < agentData.length; i++) {
					await monster.request({
						resource: 'callcenter.agent.cdrId',
						data: {
							accountId: self.accountId,
							cdrId: agentData[i].id
						},
						success: function (agent_cdr) {
							if (agent_cdr.data.custom_channel_vars.queue_id !== undefined){
								if (agent_cdr.data.custom_channel_vars.queue_id == queueId){
									
									if (agent_cdr.data.hangup_cause == "NO_ANSWER"){
										formatted_data.agents[agentId][queueId].missed_calls++;
									}else{	//Consider miss calls not greater than agent ring timeout
										if (agent_cdr.data.ringing_seconds > 0 && agent_cdr.data.billing_seconds == 0){
											formatted_data.agents[agentId][queueId].missed_calls++;
										}
									}
									formatted_data.agents[agentId][queueId].talk_time = formatted_data.agents[agentId][queueId].talk_time + agent_cdr.data.billing_seconds;
									formatted_data.agents[agentId][queueId].wait_time = formatted_data.agents[agentId][queueId].wait_time + agent_cdr.data.ringing_seconds;
									
									formatted_data.agents[agentId][queueId].total_calls++;
									formatted_data.agents[agentId][queueId].answered_calls = formatted_data.agents[agentId][queueId].total_calls - formatted_data.agents[agentId][queueId].missed_calls;
								}
							}
						}
					});
